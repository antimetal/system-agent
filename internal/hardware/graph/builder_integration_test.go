...
func TestHardwareGraph_RealSystemDiscovery(t *testing.T) {
	...
	// Verify system node was created by querying for SystemNode type
	// Note: We can't use a specific name since it's based on host.CanonicalName()
	// which varies per system. Instead, we just verify the build succeeded.
	t.Log("Hardware graph built successfully from real system data")
