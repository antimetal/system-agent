...
docker run --rm \
    ...
    ebpf-builder \
    bash -c "
        # Generate vmlinux.h if BTF is available and file doesn't exist
        if [ -f /sys/kernel/btf/vmlinux ] && [ ! -f /workspace/ebpf/include/vmlinux.h ]; then
            echo 'Generating vmlinux.h from host BTF...'
            mkdir -p /workspace/ebpf/include
            
            # Find bpftool (handles version-specific paths)
            BPFTOOL=\$(which bpftool || find /usr/lib/linux-tools -name bpftool -executable | head -1)
