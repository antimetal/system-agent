...
// getFirstOnlineCPU returns the first online CPU number, or 0 as fallback
func getFirstOnlineCPU() int {
	data, err := os.ReadFile("/sys/devices/system/cpu/online")
	if err != nil {
		return 0 // Fallback to CPU 0
	}

	cpus, err := cpuutil.ParseCPUList(strings.TrimSpace(string(data)))
...
func isPerfEventAvailable(eventType uint32, config uint64) bool {
	// Software events should always be available
	if eventType == PERF_TYPE_SOFTWARE {
		return true
	}

	// For hardware events, test if they're actually available
	cpu := getFirstOnlineCPU()
	...
	// Try to open the event on an online CPU for the current process
