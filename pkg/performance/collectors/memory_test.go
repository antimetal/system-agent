...
`
	...
	// Edge cases
	extraWhitespaceMeminfoContent = `  MemTotal:        8192000 kB  
  MemFree:         1024000 kB  
  Buffers:          256000 kB  
  Cached:          2048000 kB  
`
...
func createMemoryTestCollector(t *testing.T, meminfoContent string) *MemoryCollector {
	...
	return NewMemoryCollector(logr.Discard(), config)
}
...
func TestMemoryCollector_Constructor(t *testing.T) {
	tests := []struct {
		...
		{"default proc path", "/proc", "/proc/meminfo"},
		{"custom proc path", "/custom/proc", "/custom/proc/meminfo"},
		{"empty proc path", "", "meminfo"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			collector := NewMemoryCollector(logr.Discard(), config)
