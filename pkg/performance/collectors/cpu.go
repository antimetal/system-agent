...
func (c *CPUCollector) collectCPUStats() ([]*performance.CPUStats, error) {
	...
	for _, line := range lines {
		...
		// Store total time - utilization requires deltas between snapshots
		stats.DeltaTotal = stats.User + stats.Nice + stats.System + stats.Idle +
			stats.IOWait + stats.IRQ + stats.SoftIRQ + stats.Steal +
			stats.Guest + stats.GuestNice
		
		// Set utilization to 0 - it needs to be calculated from deltas
		stats.Utilization = 0
	...
	c.Logger().V(1).Info("Collected CPU statistics", "cpus", len(cpuStats))
	return cpuStats, nil
}
