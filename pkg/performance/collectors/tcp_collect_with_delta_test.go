...
`
		...
		// First collection should have no deltas
		assert.Nil(t, stats1.DeltaActiveOpens)
...
`
		...
		require.NotNil(t, stats2.DeltaActiveOpens)
		assert.Equal(t, uint64(50), *stats2.DeltaActiveOpens)  // 1050 - 1000
		assert.Equal(t, uint64(20), *stats2.DeltaPassiveOpens) // 520 - 500
		assert.Equal(t, uint64(2000), *stats2.DeltaInSegs)     // 102000 - 100000
		assert.Equal(t, uint64(1500), *stats2.DeltaOutSegs)    // 96500 - 95000
		assert.Equal(t, uint64(20), *stats2.DeltaRetransSegs)  // 220 - 200

		// Extended stats deltas
		assert.Equal(t, uint64(2), *stats2.DeltaSyncookiesSent)      // 7 - 5
		assert.Equal(t, uint64(1), *stats2.DeltaSyncookiesRecv)      // 4 - 3
		assert.Equal(t, uint64(1), *stats2.DeltaSyncookiesFailed)    // 3 - 2
		assert.Equal(t, uint64(2), *stats2.DeltaListenOverflows)     // 10 - 8
		assert.Equal(t, uint64(3), *stats2.DeltaListenDrops)         // 15 - 12
		assert.Equal(t, uint64(5), *stats2.DeltaTCPLostRetransmit)   // 30 - 25
		assert.Equal(t, uint64(5), *stats2.DeltaTCPFastRetrans)      // 45 - 40
		assert.Equal(t, uint64(5), *stats2.DeltaTCPSlowStartRetrans) // 35 - 30
		assert.Equal(t, uint64(10), *stats2.DeltaTCPTimeouts)        // 110 - 100

		// Verify rates are calculated (should be > 0 since we enabled rates mode)
		require.NotNil(t, stats2.ActiveOpensPerSec)
		assert.Greater(t, *stats2.ActiveOpensPerSec, 0.0)

		require.NotNil(t, stats2.InSegsPerSec)
		assert.Greater(t, *stats2.InSegsPerSec, 0.0)

		// Verify metadata
		require.NotNil(t, stats2.DeltaMetadata)
		assert.False(t, stats2.DeltaMetadata.IsFirstCollection)
		assert.False(t, stats2.DeltaMetadata.CounterResetDetected)
		assert.Greater(t, stats2.DeltaMetadata.CollectionInterval, 100*time.Millisecond)
		assert.Less(t, stats2.DeltaMetadata.CollectionInterval, time.Second)
	})
...
`
		...
		for i := 0; i < 3; i++ {
			...
			assert.Nil(t, stats.DeltaActiveOpens)
...
`
		...
		stats, ok := result.(*performance.TCPStats)
		require.True(t, ok)
		require.NotNil(t, stats.DeltaActiveOpens)

		// Deltas should be 0 due to reset detection
		assert.Equal(t, uint64(0), *stats.DeltaActiveOpens)
		assert.Equal(t, uint64(0), *stats.DeltaInSegs)
		assert.Equal(t, uint64(0), *stats.DeltaRetransSegs)

		// Reset should be detected
		require.NotNil(t, stats.DeltaMetadata)
		assert.True(t, stats.DeltaMetadata.CounterResetDetected)
	})
...
`
		...
		assert.Nil(t, stats1.DeltaActiveOpens) // No delta on first collection
		...
		assert.Nil(t, stats2.DeltaActiveOpens) // Should skip delta calculation
		...
		assert.Nil(t, stats3.DeltaActiveOpens) // Should skip delta calculation due to large interval
	})
