...
func TestTCPCollector_DeltaAwareCollector(t *testing.T) {
	...
	t.Run("delta calculation progression", func(t *testing.T) {
		...
		require.NotNil(t, secondStats.DeltaActiveOpens)
		assert.Equal(t, uint64(50), *secondStats.DeltaActiveOpens)
		require.NotNil(t, secondStats.DeltaRetransSegs)
		assert.Equal(t, uint64(25), *secondStats.DeltaRetransSegs)
		require.NotNil(t, secondStats.DeltaInSegs)
		assert.Equal(t, uint64(1000), *secondStats.DeltaInSegs)

		// Verify rate calculations (delta / 1.5s)
		require.NotNil(t, secondStats.ActiveOpensPerSec)
		assert.InDelta(t, 33.33, *secondStats.ActiveOpensPerSec, 0.01) // 50/1.5s ≈ 33.33/s

		require.NotNil(t, secondStats.RetransSegsPerSec)
		assert.InDelta(t, 16.67, *secondStats.RetransSegsPerSec, 0.01) // 25/1.5s ≈ 16.67/s

		require.NotNil(t, secondStats.InSegsPerSec)
		assert.InDelta(t, 666.67, *secondStats.InSegsPerSec, 0.01) // 1000/1.5s ≈ 666.67/s

		// Verify metadata
		require.NotNil(t, secondStats.DeltaMetadata)
		assert.Equal(t, interval, secondStats.DeltaMetadata.CollectionInterval)
		assert.False(t, secondStats.DeltaMetadata.IsFirstCollection)
		assert.False(t, secondStats.DeltaMetadata.CounterResetDetected)
	})
	...
	t.Run("counter reset detection", func(t *testing.T) {
		...
		require.NotNil(t, secondStats.DeltaActiveOpens)
		assert.Equal(t, uint64(0), *secondStats.DeltaActiveOpens)
		require.NotNil(t, secondStats.DeltaInSegs)
		assert.Equal(t, uint64(0), *secondStats.DeltaInSegs)

		// Reset should be detected
		require.NotNil(t, secondStats.DeltaMetadata)
		assert.True(t, secondStats.DeltaMetadata.CounterResetDetected)
	})
