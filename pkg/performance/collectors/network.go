...
type NetworkCollector struct {
	...
	prevStats map[string]*networkCounters
	prevTime  time.Time
	mu        sync.Mutex
}

// networkCounters stores raw counters for rate calculation
...
func NewNetworkCollector(logger logr.Logger, config performance.CollectionConfig) *NetworkCollector {
	...
	return &NetworkCollector{
		...
		prevStats:       make(map[string]*networkCounters),
	}
}
...
func (c *NetworkCollector) collectNetworkStats() ([]performance.NetworkStats, error) {
	...
	if c.prevTime.IsZero() {
		timeDelta = 0 // First collection, no rates
	}
	...
	for scanner.Scan() {
		...
		if timeDelta > 0 {
			if prev, ok := c.prevStats[ifaceName]; ok {
				// Calculate bytes/packets per second
				if stat.RxBytes >= prev.rxBytes {
					stat.RxBytesPerSec = float64(stat.RxBytes-prev.rxBytes) / timeDelta
				}
				if stat.RxPackets >= prev.rxPackets {
					stat.RxPacketsPerSec = float64(stat.RxPackets-prev.rxPackets) / timeDelta
				}
				if stat.TxBytes >= prev.txBytes {
					stat.TxBytesPerSec = float64(stat.TxBytes-prev.txBytes) / timeDelta
				}
				if stat.TxPackets >= prev.txPackets {
					stat.TxPacketsPerSec = float64(stat.TxPackets-prev.txPackets) / timeDelta
				}
			}
		}

		// Store current counters for next calculation
		c.prevStats[ifaceName] = &networkCounters{
	...
	// Update time for next rate calculation
	c.prevTime = now
