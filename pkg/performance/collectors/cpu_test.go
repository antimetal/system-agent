...
`,
			...
			validate: func(t *testing.T, stats []*performance.CPUStats) {
				...
				// Total time = 1234 + 56 + 789 + 10000 + 200 + 30 + 40 + 50 + 60 + 70 = 12529
				// Utilization = 100 - (10000 / 12529 * 100) ≈ 20.18517...%
				assert.InDelta(t, 20.18517, stats[0].Utilization, 0.1)
...
`,
			...
			validate: func(t *testing.T, stats []*performance.CPUStats) {
				...
				// Total time = 1234 + 56 + 789 + 10000 + 200 + 30 + 40 = 12349
				// Utilization = 100 - (10000 / 12349 * 100) ≈ 19.02178...%
				assert.InDelta(t, 19.02178, stats[0].Utilization, 0.1)
			},
		},
		{
			name:        "empty file",
...
`,
			...
			validate: func(t *testing.T, stats []*performance.CPUStats) {
				...
				// Check aggregate shows 20% utilization
				// Total = 1000 + 0 + 1000 + 8000 = 10000
				// Utilization = 100 - (8000 / 10000 * 100) = 20%
				assert.InDelta(t, 20.0, stats[0].Utilization, 0.001)

				// Check individual CPUs
				for i := 1; i <= 10; i++ {
					assert.Equal(t, int32(i-1), stats[i].CPUIndex)
					assert.InDelta(t, 20.0, stats[i].Utilization, 0.001)
				}
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.True(t, ok, "result should be []*performance.CPUStats")
...
func TestCPUCollector_Properties(t *testing.T) {
	config := performance.CollectionConfig{
		HostProcPath: "/proc",
	}
	logger := logr.Discard()
	...
	assert.Equal(t, performance.MetricTypeCPU, collector.Type())
	assert.Equal(t, "CPU Statistics Collector", collector.Name())
	...
	assert.Equal(t, "2.6.0", caps.MinKernelVersion)
}
