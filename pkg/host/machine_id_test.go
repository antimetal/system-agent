...
func TestGetMachineID(t *testing.T) {
	...
	assert.NotEqual(t, "unknown", id, "Machine ID should not be 'unknown' on a real Linux system")
...
func TestGetMachineID_Consistency(t *testing.T) {
...
func TestGetMachineID_ValidFormat(t *testing.T) {
	id := GetMachineID()
...
func TestGetSystemUUID(t *testing.T) {
	...
	if uuid != "" {
		...
		t.Log("System UUID not available (may require root or DMI may not be present)")
	}
}

func TestGetSystemUUID_Consistency(t *testing.T) {
...
func TestMachineID_FallbackBehavior(t *testing.T) {
	...
	dmiData, dmiErr := os.ReadFile("/sys/class/dmi/id/product_uuid")
	hostname, _ := os.Hostname()
	
	var source string
	if machineIDErr == nil && len(machineIDData) > 0 {
		// Check if our ID matches machine-id
		machineIDStr := string(machineIDData)
		if len(machineIDStr) > 0 {
			source = "/etc/machine-id"
		}
	}
	
	if source == "" && dmiErr == nil && len(dmiData) > 0 {
		// Check if our ID matches DMI UUID
		dmiStr := string(dmiData)
		if len(dmiStr) > 0 {
			source = "/sys/class/dmi/id/product_uuid"
		}
	}
	
	if source == "" && hostname != "" && id == hostname {
		source = "hostname"
	}
